HomePage <- function(id) {
  ...
  tags$div(
    ...
    tags$div(
      style = "display:flex; justify-content:center; place-items:center; width:900px; max-width:100%;",
      tags$div(
        class = 'left',
        h3('Añadir producto'),
    ...
    function(input, output, session) {
      ...
      observeEvent(input$addProducto, {
        ...
        if(newp$status == 1) {errors('No se ha podido crear el producto. Por favor revise todos los campos')} else {
          ...
          if(pId$status == 0) {
            showNotification(ui = tags$h4('Se ha creado un nuevo producto'), type = 'message')
          } else {
